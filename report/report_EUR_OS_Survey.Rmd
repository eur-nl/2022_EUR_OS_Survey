...
A total of `r sum(EUR_OS_incomplete$n)` respondents started the survey, and `r pull(filter(EUR_OS_incomplete, survey == "complete"), n)` completed it. Separate response rates per school and position can be found in Table \@ref(tab:table1) and Table \@ref(tab:table2), respectively.
...
(ref:table2) Response rate per position

(ref:table2-footnote) NA indicates no response.
...
EUR_OS_incomplete_position %>%
  select(-perc) %>% 
  rename("completed survey" = lab_perc) %>% 
  kable_custom(caption = "(ref:table2)") %>% 
  footnote(
    general = "(ref:table2-footnote)",
    footnote_as_chunk = TRUE
  )
