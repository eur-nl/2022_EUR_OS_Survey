...
###################################
# CONTINUE FROM HERE
# COUNT NUMBER OF RESPONSES TO THE QUESTION
...
# SEPARATELY PER SCHOOL & CONDITION
###################################
...
# EUR_OS_open_access <-
#   EUR_OS_cluster1 %>% 
#   filter(question %in% c("School", "Position")
...
EUR_OS_open_access_importance <-
  EUR_OS_cluster1 %>% 
  filter(question == c("School", "Position",
                         "In your opinion, how important is Open Access for your work?")
                         ) %>%
  droplevels() %>%
  pivot_wider(
  ...
  summarize(
    n = n(),
    lab_perc = paste(round(n / sum(n) * 100, 2), "%", sep = ""),
    .groups = "keep"
  )
