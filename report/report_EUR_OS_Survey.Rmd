...
EUR_OS_incomplete_position <-
  ...
  mutate(Position = gsub("Other_.*", "Other", Position)) %>% # group all other responses in "Other"
  group_by(Position) %>%
  summarize(
    n = n(),
    perc = round(length(which(Finished == TRUE))/length(Finished) * 100, 2),
...
EUR_OS_incomplete_department <-
  ...
  mutate(
    value_1 = recode(
      ...
      "Erasmus School of Law (ESL)" = "ESL",
      "International Institute of Social Studies (ISS)" = "ISS",
      "Public Administration,Sociology" = "Other", # double affiliation categorized as "Other"
    ),
    value_1 = gsub("Other_.*", "Other", value_1), # group all other responses in "Other"
  ) %>% 
  select(-c(value_2, value_3, value_4, value_5, value_6, value_7, value_8, value_9)) %>% # delete columns with no data
  pivot_wider(
    id_cols = c(participant, Finished),
    names_from = question,
    values_from = value_1
