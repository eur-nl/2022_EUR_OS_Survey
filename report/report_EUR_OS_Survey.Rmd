...
EUR_OS_incomplete_school <-
  ...
  filter(
    question == "School" &
      (value == "value_1" & !is.na(item))
    ) %>% 
  mutate(
    item = recode( # use school abbreviations, for better readability
      factor(item),
...
EUR_OS_incomplete_department <-
  EUR_OS_cluster0 %>% 
  filter(
    question %in% c("School", "Department") &
      (value == "value_1" & !is.na(item))
    ) %>%
  mutate(
    item = recode(
      ...
      "Strategy" = "Other",
      "Public Administration,Sociology" = "Other",
      "Support Staff" = "NA"
    )
  ) %>%
  pivot_wider(
...
# EUR_OS_open_access <-
#   read_csv(
#     here("data", "preproc", "cluster_1.csv"),
#     col_types = c("ffddc")
#   )
